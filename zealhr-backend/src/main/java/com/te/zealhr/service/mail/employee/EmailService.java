package com.te.zealhr.service.mail.employee;
...
public class EmailService {
	...
	public Integer sendMailWithCC(MailDto mailDto) {
		...
		try {
			...
			helper.setSubject(mailDto.getSubject());
			helper.setTo(mailDto.getTo());
			mailDto.getCc().stream().forEach(a -> {
				try {
					message.addRecipient(RecipientType.BCC, new InternetAddress(a));
				} catch (MessagingException e) {
					e.printStackTrace();
					log.error("www.fast2sms.com with error :", e.getMessage());
				}
			});
			emailSender.send(message);
			return HttpStatus.OK.value();
