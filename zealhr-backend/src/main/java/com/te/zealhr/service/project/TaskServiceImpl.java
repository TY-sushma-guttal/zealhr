...
import static com.te.zealhr.common.hr.HrConstants.COMPANY_INFORMATION_FETCHED_SUCCESSFULLY;
import static com.te.zealhr.common.hr.HrConstants.COMPANY_INFORMATION_NOT_PRESENT;
import static com.te.zealhr.common.hr.ProjectConstants.COMPLETED;
import static com.te.zealhr.common.hr.ProjectConstants.IN_PROGRESS;
import static com.te.zealhr.common.hr.ProjectConstants.PROJECT_DETAILS_NOT_FOUND;
import static com.te.zealhr.common.hr.ProjectConstants.PROJECT_INFORMATION_FETCHED_SUCCESSFULLY;
import static com.te.zealhr.common.hr.ProjectConstants.TASK_DETAILS_NOT_FOUND;
import static com.te.zealhr.common.hr.ProjectConstants.TASK_UPDATED_SUCCEESSFULLY;
...
import static com.te.zealhr.common.project.ProjectManagementConstants.STATUS_UNASSIGNED;
...
import com.te.zealhr.dto.project.mongo.TaskHistroy;
...
import com.te.zealhr.entity.project.mongo.ProjectMilestoneDeliverables;
...
import com.te.zealhr.repository.project.mongo.MilestoneRepository;
...
public class TaskServiceImpl implements TaskService {
	...
	public TaskDetailsDTO assignTaskToEmployee(AssignTaskDTO assignTaskDTO) {
		...
		if (projectDetails.getCompanyInfo().getCompanyId().equals(assignTaskDTO.getCompanyId())) {
			...
			taskDetailsDTO.setAssignedEmployeeName(personalInfo.getFirstName());
	...
	public TaskDetailsDTO markAsCompleted(Long companyId, String taskId, Long projectId) {
		...
		if (projectTaskDetails.getAssignedEmployee() != null) {
			...
			if ((employeeName != null) && (!employeeName.isEmpty())) {
				taskDetailsDTO.setAssignedEmployeeName(employeeName.get(0).getFirstName());
			}
		}
		...
		List<TaskDetailsDTO> taskDetailsDTOList = findByProjectIdAndStatus.stream().map(a -> {
			...
			if (a.getStatus().equalsIgnoreCase(COMPLETED) || (a.getStatus().equalsIgnoreCase(IN_PROGRESS))) {
				...
				if ((employeeDetails != null) && (!employeeDetails.isEmpty())) {
					details.setAssignedEmployeeName(employeeDetails.get(0).getFirstName());
				}
			}
			...
			if (employeeDetails != null) {
				details.setCreatedByName(employeeDetails.getFirstName());
			}
			return details;
		...
		if (status.equalsIgnoreCase("all")) {
			...
			for (ProjectTaskDetails task : taskDetail) {
				arrayList.add(new TaskDetailsDTO(task.getId(), task.getMileStoneId(), null, task.getSubMilestoneId(),
						...
						companyInfo.getCompanyId(), task.getProjectId(), employeedetails.get(0).getFirstName(),
						task.getComment()));
			}
		...
		for (ProjectTaskDetails a : findByProjectIdAndStatus) {
			...
			if (now.isAfter(a.getEndDate())) {
				...
				if (!employeeDetail.isEmpty()) {
					details.setAssignedEmployeeName(employeeDetail.get(0).getFirstName());
				}
