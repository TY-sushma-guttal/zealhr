...
import static com.te.zealhr.common.admin.EmployeeAdvanceSalaryConstants.INVALID_STATUS_PLEASE_PROVIDE_VALID_STATUS;
...
import static com.te.zealhr.common.admin.EmployeeAdvanceSalaryConstants.SOMETHING_WENT_WRONG;
					...
					return optional
							...
							.map(t -> {
								...
								if (employeeAdvanceSalary.getEmployeePersonalInfo().getExpoToken() != null) {
									pushNotificationService.pushMessage("zealhr", "Advance salary request for the amount "+employeeAdvanceSalary.getAmount()+" is Rejected by Admin",
							...
							.orElseGet(() -> optional.filter(
									...
									.map(o -> {
										...
										notificationServiceImpl.saveNotification("Advance salary request for the amount "+employeeAdvanceSalary.getAmount()+" is Approved by Admin",
												employeeAdvanceSalary.getEmployeePersonalInfo().getEmployeeInfoId());
										if (employeeAdvanceSalary.getEmployeePersonalInfo().getExpoToken() != null) {
											pushNotificationService.pushMessage("zealhr", "Advance salary request for the amount "+employeeAdvanceSalary.getAmount()+" is Approved by Admin",
													employeeAdvanceSalary.getEmployeePersonalInfo().getExpoToken());
											}
