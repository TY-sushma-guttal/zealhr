...
import com.te.zealhr.dto.tally.OrderItemDTO;
import com.te.zealhr.dto.tally.PayrollDetailsDTO;
...
// one iteration of  for loop in if condition
			for (OrderItemDTO orderItemDTO2 : addPurchaseDetails) {

				List<CompanyPurchaseOrder> findBySubject = companyPurchaseOrderRepository
						.findBySubjectAndCompanyInfoCompanyId(orderItemDTO.getLedgerName() + " (" + orderItemDTO.getStockGroup() + ")",companyInfo.getCompanyId());
	...
	public List<OrderItemDTO> savePurchaseDetails(MultipartFile transaction, String companyName) {
		...
		for (int i = 0; i < taxXML.getLength(); i++) {
			...
			String itemName = taxElement.getElementsByTagName("LEDGERNAME").item(0).getTextContent();

			if (itemName.equalsIgnoreCase("cgst")) {
			...
			if (itemName.equalsIgnoreCase("sgst")) {
				String sgstAmount = taxElement.getElementsByTagName("AMOUNT").item(0).getTextContent();
				sgstTotal = (new BigDecimal(sgstAmount)).doubleValue();
				if ((new BigDecimal(sgstAmount)).compareTo(new BigDecimal("0")) < 0) {

					sgstTotal = (new BigDecimal(sgstAmount).multiply(new BigDecimal("-1")).doubleValue());
				}
			}
		...
		Double cgstPercentage = (100 * cgstTotal) / totalPayableAmount;

		Double sgstPercentage = (100 * sgstTotal) / totalPayableAmount;

		final Double cgstPercentageFinal = cgstPercentage;

		final Double sgstPercentageFinal = sgstPercentage;

		purchaseOrderItemDTOs.forEach(x -> {
			x.setTaxTotal(BigDecimal
					.valueOf((x.getAmount().doubleValue() * cgstPercentageFinal) / 100
							+ (x.getAmount().doubleValue() * sgstPercentageFinal) / 100)
	...
	public List<OrderItemDTO> saveSales(MultipartFile master, MultipartFile transaction, CompanyInfo companyInfo) {
		...
		for (OrderItemDTO orderItemDTO : saveSalesDetails) {
			...
			// one iteration of for loop in if condition
			for (OrderItemDTO orderItemDTO2 : saveSalesDetails) {
