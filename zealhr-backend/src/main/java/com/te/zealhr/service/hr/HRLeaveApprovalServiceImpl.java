...
import static com.te.zealhr.common.admin.EmployeeLeaveDetailsConstants.EMPLOYEE_NOT_FOUND;
import static com.te.zealhr.common.admin.EmployeeLeaveDetailsConstants.HR;
import static com.te.zealhr.common.admin.EmployeeLeaveDetailsConstants.HR_APPROVAL_NOT_REQUIRED;
...
import static com.te.zealhr.common.admin.EmployeeLeaveDetailsConstants.RM;
...
public class HRLeaveApprovalServiceImpl implements HRLeaveApprovalService {
	...
	@Override
	public List<EmployeeLeaveInfoDTO> getLeaveDetailsByStatus(Long companyId, Long employeeInfoId,
		...
		List<EmployeeLeaveApplied> employeeLeaveAppliedList = employeeLeaveAppliedRepository
				.findByEmployeePersonalInfoCompanyInfoCompanyIdAndEmployeePersonalInfoEmployeeInfoIdInAndStartDateBetween(
						companyId, employeeInfoIdList, startEndDate.get(0), startEndDate.get(1));

		employeeLeaveAppliedList = filterLeaveBasedOnCondition(employeeLeaveAppliedList, levels);
		...
		return employeeLeaveAppliedList.stream().filter(leave -> leave.getEmployeePersonalInfo() != null
				&& leave.getEmployeePersonalInfo().getEmployeeOfficialInfo() != null).map(leave -> {
					...
					return EmployeeLeaveInfoDTO.builder().leaveAppliedId(leave.getLeaveAppliedId())
							.name(employee.getFirstName()).designation(officialInfo.getDesignation())
							.employeeInfoId(employee.getEmployeeInfoId()).employeeId(officialInfo.getEmployeeId())
							.leaveOfType(leave.getLeaveOfType()).startDate(leave.getStartDate())
							.status(leave.getStatus()).endDate(leave.getEndDate())
	...
	private List<EmployeeLeaveApplied> filterLeaveBasedOnCondition(List<EmployeeLeaveApplied> employeeLeaveAppliedList,
			List<String> levels) {
		if (levels.get(0).equalsIgnoreCase(HR)) {
			return employeeLeaveAppliedList;
		} else {
			return employeeLeaveAppliedList.stream().filter(leaves -> (leaves.getApprovedBy().keySet().contains(RM)))
					.collect(Collectors.toList());
		}
	}
	...
	public EmployeeLeaveInfoDTO getLeaveDetailsById(Long leaveAppliedId, Long employeeInfoId, Long companyId) {
		return employeeLeaveAppliedRepository
				...
				.filter(Objects::nonNull).map(leave -> {
					...
					String reportingManagerName = (reportingManager == null) ? null : reportingManager.getFirstName();
