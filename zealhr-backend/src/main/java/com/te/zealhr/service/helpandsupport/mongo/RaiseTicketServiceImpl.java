...
import com.te.zealhr.dto.hr.mongo.TicketHistoryDTO;
...
import com.te.zealhr.repository.admindept.CompanyHardwareItemsRepository;
...
import com.te.zealhr.repository.superadmin.mongo.SuperAdminTicketsRepository;
import com.te.zealhr.audit.Audit;
import com.te.zealhr.audit.common.db.DBConstants;
import com.te.zealhr.util.CacheStore;
import com.te.zealhr.util.Generator;
...
public class RaiseTicketServiceImpl implements RaiseTicketService {
	...
	@Autowired
	private CacheStore<CompanyRuleInfo> companyRuleInfoCache;
		...
		if (getfrs != null && companyInfo != null) {
			...
			for (ReportingManagerDto reportingManagerDto : getfrs) {
				...
				arrayList.add(new ReportingManagerDto(reportingManagerDto.getEmployeeId(),
						reportingManagerDto.getFirstName()));
			}
		}
		return arrayList;
	}
...
//				.forEach(x -> idAndName.put(x.getEmployeeInfoId(), x.getFirstName()));
//
...
//	}
	...
	public List<CompanyTicketDto> getAllTickets(Long companyId, CompanyTicketDto companyTicketDto) {
		...
		// return getAllTicket(companyInfo.getCompanyId(),
		// companyTicketDto.getMonitoringDepartment());
		return null;
	}
...
//	}
	...
	private CompanyTicketDto hrTicketDetails(Long companyId, String id, String department) {
		...
		//
		companyTicketDto.setTicketOwner(findById.get().getFirstName());
		...
		for (TicketHistroy ticketHistroy : ticketHistroys) {
			for (EmployeePersonalInfo employeePersonalInfo2 : employeeInfo) {
				if (employeePersonalInfo2.getEmployeeInfoId().equals(ticketHistroy.getBy())) {
					...
					ticketHistroy.setEmployeeName(employeePersonalInfo2.getFirstName());
	...
	@Override
	public List<TicketCategoryDTO> getDepartmentCategories(Long companyId) {
		CompanyRuleInfo companyRuleInfo = companyRuleInfoCache.get(companyId.toString());
		...
		for (Entry<String, HashMap<String, String>> departmentTicketCategories : companyRuleInfo.getTicketCategories()
				.entrySet()) {
			ticketCategoryDTOList.add(TicketCategoryDTO.builder()
					.defaultCategories(new ArrayList<>(departmentTicketCategories.getValue().keySet()))
					.department(departmentTicketCategories.getKey()).build());
		}
