...
import static com.te.zealhr.common.project.ProjectManagementConstants.PROJECT_SAVED;
import static com.te.zealhr.common.project.ProjectManagementConstants.PROJECT_UPDATED;
...
public class ProjectDetailsServiceImpl implements ProjectDetailsService {
	...
	public List<ProjectDetailsBasicDTO> getAllProjectDetailsBasicByCompanyId(Long companyId) {
		...
		projectDetailsRepository.findByCompanyInfoCompanyId(companyId).forEach(projectDetail -> {
			...
			String projectOwner = "";
			String clientName = "";
			if (Optional.ofNullable(projectDetail.getProjectManager()).isPresent() && Optional.ofNullable(projectDetail.getProjectManager().getFirstName()).isPresent()) {
				projectManager = String.format("%s", projectDetail.getProjectManager().getFirstName());
...
//					projectOwner = String.format("%s", employeePersonalInfo.getFirstName());
//				}
//			} else projectOwner = null;
		...
		empNames.stream().forEach(e->
			ownerInfo.put(e.getEmployeeInfoId(),e.getFirstName())
		);
