...
import com.te.zealhr.auth.filter.JwtAuthenticationFilter;
import com.te.zealhr.exception.auth.CustomAccessDeniedException;
import com.te.zealhr.exception.auth.CustomAuthenticationEntryPoint;
...
//		.antMatcher("api/v1/layover/transportation/registration").authorizeRequests().anyRequest().permitAll();
		http.cors().configurationSource(request -> {
		 ...
		 http.csrf().disable().authorizeHttpRequests().antMatchers(
				...
				"/api/v1/web-contact/**",
				"api/v1/employee/send-otp",
				"api/v1/employee/resend-otp",
				"api/v1/employee/verify-login-otp"
				).permitAll()
