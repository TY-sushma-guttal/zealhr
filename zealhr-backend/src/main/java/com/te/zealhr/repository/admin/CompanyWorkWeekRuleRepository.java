...
public interface CompanyWorkWeekRuleRepository extends JpaRepository<CompanyWorkWeekRule,Long> {
	...
	@Query(value = "select new com.te.zealhr.dto.admin.CompanyWorkWeekRuleNameDTO(cwwr.workWeekRuleId,cwwr.ruleName)from CompanyWorkWeekRule cwwr where cwwr.companyInfo.companyId=?1")
	public Optional<List<CompanyWorkWeekRuleNameDTO>> getWorkWeekByCompanyId(Long companyId);

	@Query(value="select new com.te.zealhr.responsedto.admin.CompanyWorkWeekRuleResponseDto(cwwr.workWeekRuleId, cwwr.ruleName, cwwr.isDefault) from CompanyWorkWeekRule cwwr where cwwr.companyInfo.companyId=:companyId")
