...
public interface CompanyAdminDeptTicketsRepo extends MongoRepository<CompanyAdminDeptTickets, String> {
	...
	List<CompanyAdminDeptTickets> findByCompanyIdAndCreatedBy(Long companyId, Long employeeInfoId);

	List<CompanyAdminDeptTickets> findByCompanyIdAndCreatedByAndCreatedDateBetween(Long companyId, Long employeeInfoId,
			LocalDateTime atStartOfDay, LocalDateTime atStartOfDay2);
	
	List<CompanyAdminDeptTickets> findByCreatedByInAndTicketHistroysStatusNotIn(List<Long> employeeInfoIdList, String status);
}
