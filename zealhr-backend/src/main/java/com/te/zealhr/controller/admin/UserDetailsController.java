...
import com.te.zealhr.service.admin.UserDetailsService;
...
public class UserDetailsController extends BaseConfigController {
	...
	@PostMapping("employee/bulk-upload")
	public ResponseEntity<SuccessResponse> employeeBlukUpload(
			@RequestBody List<EmployeeUploadDTO> employeeUploadDTOList) {
		String employeeBlukUpload = userDetailsService.employeeBlukUpload(employeeUploadDTOList, getCompanyId(), getUserId());
		return new ResponseEntity<>(new SuccessResponse(false, employeeBlukUpload, employeeBlukUpload), HttpStatus.OK);
	}
