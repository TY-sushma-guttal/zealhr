...
@CrossOrigin(origins = "*")
public class CompanyEventDetailsController extends BaseConfigController {
...
// Controller for fetching department names for department dropdown ..
	...
	ResponseEntity<SuccessResponse> fetchDepartment() {
		...
		List<EventManagementDepartmentNameDTO> departmentInfo = companyEventDetailsService
				.fetchDepartmentNames(getCompanyId());
	...
	// year
	@PostMapping("/displayeventslist")
	ResponseEntity<SuccessResponse> getEvents(@RequestBody CanlenderRequestDTO calCanlenderRequestDTO) {
		List<EventManagementDisplayEventDTO> eventList = companyEventDetailsService.displayEventList(calCanlenderRequestDTO,
				getCompanyId());
...
// Controller for adding the New Event
	...
	ResponseEntity<SuccessResponse> addEvent(@RequestBody EventManagementAddEventDTO addEventdto,
		...
		if (eventDetails != null) {
			...
			return new ResponseEntity<>(SuccessResponse.builder().data(eventDetails).error(false)
					.message("New Event Created Successfully").build(), HttpStatus.OK);
		} else {
	...
	ResponseEntity<SuccessResponse> addWinner(@RequestBody EventManagementAddWinnerDTO addWinnerdto,
		...
		if (winnerDetails != null) {
			return new ResponseEntity<>(SuccessResponse.builder().data(winnerDetails).error(false)
					.message("Winner Details Updated Successfully").build(), HttpStatus.OK);
	...
	ResponseEntity<SuccessResponse> deleteEvent(@PathVariable("eventId") Long eventId) {
		...
		return new ResponseEntity<>(
				SuccessResponse.builder().data(null).error(false).message("Event deleted successfully").build(),
				HttpStatus.OK);
	...
	ResponseEntity<SuccessResponse> addWinner(@RequestBody EventManagementEditEventDTO editEventdto,
		...
		if (winnerDetails != null) {
			return new ResponseEntity<>(SuccessResponse.builder().data(winnerDetails).error(false)
					.message("Event Updated Successfully").build(), HttpStatus.OK);
	...
	ResponseEntity<SuccessResponse> participantslist(@PathVariable("eventId") Long eventId) {
		...
		List<EventManagementParticipantslistDTO> participantsList = companyEventDetailsService
